...
public class myTrainingPhrases {
    ...
    public myTrainingPhrases copy() {
        myTrainingPhrases copy = new myTrainingPhrases();
        for(var entry : this.getTrainingPhrases().entrySet()) {
            copy.addTrainingPhrase(entry.getValue().getSentence().getSentence());
        }

        copy.setNullTrainingPhrase(this.getNullTrainingPhrase().copy());
